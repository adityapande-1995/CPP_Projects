...
void Arc(float cx, float cy, float r, int num_segments, float startangle, float endangle){
   GLfloat mat_amb[] = { 1.0, 1.0, 1.0, 1.0 };
   GLfloat mat_spec[] = { 1.0, 1.0, 1.0, 1.0 };
   glMaterialfv(GL_FRONT, GL_AMBIENT, mat_amb);
   glMaterialfv(GL_FRONT, GL_SPECULAR, mat_amb);
   glLineWidth(0.8); 
	...
	for(int ii = 0; ii < num_segments; ii++) 
	{ 
		float theta = startangle + (endangle - startangle) * float(ii) / float(num_segments);//get the current angle 
...
void Sphere(float r, Colour c = Colour{0.0, 0.0, 1.0}){
   GLfloat mat_amb[] = { c.r, c.g, c.b, 1.0 };
   GLfloat mat_spec[] = { c.r, c.g, c.b, 1.0 };
   glMaterialfv(GL_FRONT, GL_AMBIENT, mat_amb);
   glMaterialfv(GL_FRONT, GL_SPECULAR, mat_amb);
